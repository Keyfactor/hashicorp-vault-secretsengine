...
func getCAId(ctx context.Context, req *logical.Request, b *backend) (string, error) {
	...
	b.Logger().Debug("About to connect to " + config["host"] + "for ca retrieval")
...
func fetchCertFromKeyfactor(ctx context.Context, req *logical.Request, b *backend, kfCertId string, includeChain bool) (string, error) {
	...
	// Build request
	url := config["protocol"] + "://" + host + "/KeyfactorAPI/Certificates/Download"
	b.Logger().Debug("url: " + url)
	...
	// Send request and check status
	b.Logger().Debug("About to connect to " + config["host"] + "for cert retrieval")
	res, err := http.DefaultClient.Do(httpReq)
	if err != nil {
